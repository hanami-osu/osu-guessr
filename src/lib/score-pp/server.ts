...
export async function getScorePpRoundForUser(
    userId: number,
    sessionId: string,
    round: number,
    ...
    const parsedExclusions = exclusionsSchema.parse(exclusions);
        ...
        try {
            ...
            const pair = run.multiplayer_lobby_id
                ? await getOrCreateMultiplayerRound<ScorePpPairSnapshot | null>(
                      `${normalizeLobbyCode(run.multiplayer_lobby_id)}:${run.multiplayer_match_id}`,
                      parsedRound,
                      selectPair,
                  )
                : await selectPair();
            ...
            if (run.multiplayer_lobby_id) {
                await updateMultiplayerProgress(run.multiplayer_lobby_id, userId, {
                    points: nextRun.total_points,
                    round: parsedRound,
                    finished: false,
                }, run.multiplayer_match_id ?? undefined).catch((error) => console.error("Failed to update multiplayer progress:", error));
            }
            return {
