            ...
            <div>
                ...
                <div className="max-w-3xl">
                    {announcements.length === 0 ? (
                        <div className="rounded-xl bg-card/60 px-4 py-10 text-sm text-muted-foreground sm:px-6">No announcements</div>
                    ) : (
                        <div className="space-y-4">
                            {announcements.map((a, index) => {
                                const slug = announcementSlug(a.title, a.id);
                                return (
                                    <article id={slug} key={a.id} className="scroll-mt-24 rounded-xl bg-card/60 px-4 py-6 sm:px-6 sm:py-7">
                                        <div className="mb-3 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                                            <div className="flex min-w-0 items-center gap-2">
                                                ...
                                                <h2 className="text-base font-semibold tracking-tight text-foreground">
                                                    <a href={`#${slug}`} className="underline decoration-border underline-offset-4 hover:decoration-primary">
                                                        {a.title}
                                                    </a>
                                                </h2>
                                            </div>
